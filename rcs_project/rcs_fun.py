...
def hexagon(x, y, hex_size):
    """Generates the coordinates of each corners of a hexagon 
...
def hexagonal_grid_png1(img, hex_size):
    ...
    for i in range(hex_x):
        ...
        for j in range(hex_y):
            if (i + j) % 2 == 0:
                color = get_color(
                    img, (x + hex_size / 2, y + hex_size * np.sqrt(3) / 2))
                draw.polygon(
                    hexagon(
                        x,
                        y,
...
def hexagonal_grid_png2(img, hex_size):
    ...
    for i in range(hex_x):
        ...
        for j in range(hex_y):
            if (i + j) % 2 == 0:
                color = get_color2(
                    img, (x + hex_size / 2, y + hex_size * np.sqrt(3) / 2), hex_size)
                draw.polygon(
                    hexagon(
                        x,
                        y,
