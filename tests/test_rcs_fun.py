...
def test_hexagon():
    ...
    assert round(np.mean(corners), 5) == round(np.mean([x + size/2, y + size*np.sqrt(3)/2]), 5) # Checks if the center of the hexagon is correct
...
def test_hex_grids():
    ...
    assert fun.hexagonal_grid_png1(img, 10) == fun.hexagonal_grid_png2(img, 10) # Checks if the function are operating well
    # Test image is blank, so both functions should return the same image
    assert fun.hexagonal_grid_svg1(img, 10) == fun.hexagonal_grid_svg2(img, 10) # Checks if the function are operating well